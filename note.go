...
type Note struct {
	...
	CreaterAt   time.Time `json:"created_at,omitempty"`
...
func (n Note) Create() error {
	...
	// Nos aseguramos de cerrar el recurso antes de finalizar la función gracias a defer.
	defer stmt.Close()

	// Ejecutamos la petición pasando los datos correspondientes.
	// El orden es importante, corresponde con los “?” del string q.
	r, err := stmt.Exec(n.Title, n.Description, time.Now())
	if err != nil {
		return err
	}

	// Confirmamos que una fila fuera afectada, debido a que insertamos un registro en la tabla. En caso contrario devolvemos un error.
	if i, err := r.RowsAffected(); err != nil || i != 1 {
		return errors.New("ERROR: Se esperaba una fila afectada")
	}

	// Si llegamos a este punto consideramos que todo el proceso fue exitoso y retornamos un nil para confirmar que no existe un error.
	return nil
}

func (n *Note) GetAll() ([]Note, error) {
	db := GetConnection()
	q := `SELECT id, title, description, created_at, updated_at FROM notes`
...
func (n Note) Update() error {
	db := GetConnection()
	q := `UPDATE notes set title=?, description=?, updated_at=? WHERE id=?`
